...
def main():
    ...
    scheduler= ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=10, verbose=False, threshold=0.0001,
    ...
    for epoch in range(epochs):
        ...
        if epoch % 1 == 0:
            ...
            if val_acc > best_acc:
                best_epoch = epoch
                best_acc = val_acc
                torch.save(model.state_dict(), 'SE_Resnest50_0429.mdl')
   ...
   # val_accs_y2.to_csv("val_accs_y2_2021_0426.csv")
    ...
    plt.savefig("SE_Resnest50_0429_accuracy_loss.pdf")
    plt.savefig("SE_Resnest50_0429_accuracy_loss.svg")
    plt.show()

   # model.load_state_dict(torch.load('Resnest50_0426.mdl'))
    model.load_state_dict(torch.load('SE_Resnest50_0429.mdl'))
